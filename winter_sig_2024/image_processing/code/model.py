...
class RNN_ENCODER(nn.Module):
    ...
    def init_hidden(self, bsz):
        ...
        # 최근 PyTorch에서는 data 대신 detach()를 사용하는 것이 권장된다고 하여 이 부분 수정
        weight = next(self.parameters()).detach()
        
        # Variable은 예전 번전에서 자동 미분을 활성화하는 역할을 했는데 현재 PyTorch 버전에서는 Vairable을 사용할 필요가 없다고 함
        # new_zeros()는 new().zero_()와 같은 기능을 하지만 더 직관적이고 최신 코드 스타일에 맞음
